package castro.jhostin.myfeelings

import androidx.appcompat.app.AppCompatActivity
import android.os.Bundle
import android.util.Log
import android.widget.Toast
import castro.jhostin.myfeelings.utilities.CustomBarDrawable
import castro.jhostin.myfeelings.utilities.CustomCircleDrawable
import castro.jhostin.myfeelings.utilities.Emociones
import castro.jhostin.myfeelings.utilities.JSONFile
...
class MainActivity : AppCompatActivity() {
    ...
    fun iconoMayoria(){
        ...
        if(happy>veryHappy && happy>neutral && happy>sad && happy>verySad){
            icon.setImageDrawable(resources.getDrawable(R.drawable.ic_happy_40dp))
        }
        if(veryHappy>happy && veryHappy>neutral && veryHappy>sad && veryHappy>verySad){
            icon.setImageDrawable(resources.getDrawable(R.drawable.ic_veryhappy_40dp))
        }
        if(neutral>veryHappy && neutral>happy && neutral>sad && neutral>verySad){
            icon.setImageDrawable(resources.getDrawable(R.drawable.ic_neutral_40dp))
        }
        if(sad>veryHappy && sad>happy && sad>neutral && sad>verySad){
            icon.setImageDrawable(resources.getDrawable(R.drawable.ic_sad_40dp))
        }
        if(verySad>veryHappy && verySad>happy && verySad>neutral && verySad>sad){
            icon.setImageDrawable(resources.getDrawable(R.drawable.ic_verysad_40dp))
        }
    }
